...
export async function POST(req: Request) {
  ...
  for (let item of cartItem as TCartItem[]) {
    ...
    let { status, res } = await handlePurchase({
      item,
  ...
  if (PurchaseRequestRejected.length > 0) {
    ...
    if (!SendMailSuccess || SendMailUnkownError) {
      return response({
        status: 500,
        res: { message: "Error Occured while sending email" },
      });
    }

    return response({
      status: 200,
      res: { message: "mail sucessfully sent " },
    });
  }
  ...
  if (PurchaseRequestSuccess.length > 0) {
    const { SendMailUnkownError, SendMailSuccess } = await sendMail({
      message: `<h1>Your order for follownig items was successfull for cartId: ${
        PurchaseRequestSuccess[0].cartId
      } </h1>:
