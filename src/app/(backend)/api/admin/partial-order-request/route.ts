import { findPurhaseRequestById } from "@/app/services/api/admin/product/findPurchaseRequestById";
import { handleAcceptedPartialOrderItem } from "@/app/services/api/admin/product/handleAcceptedPartialOrderItem";
import { handleRejectPartialOrderItem } from "@/app/services/api/admin/product/handleRejectPartialOrderItem";
...
export async function POST(req: Request) {
  try {
    ...
    const {
      PartialOrderItem,
      PartialOrderItemNotFoundError,
      RequestIdNotFoundError,
    } = await findPurhaseRequestById({ reqId: partialRequestItem.id });
    ...
    if (confirmation === false || confirmation === "false") {
      if (PartialOrderItem) {
        const { Success, RejectPartialOrderItemUnknownError } =
          await handleRejectPartialOrderItem({
        ...
        if (Success && !RejectPartialOrderItemUnknownError) {
          const { SendMailSuccess, SendMailUnkownError } = await sendMail({
            message: "Your request which had been pending has been rejected",
            email: PartialOrderItem.userEmail!,
          });

          if (!SendMailSuccess || SendMailUnkownError) {
            return response({
              status: 403,
              res: { message: "Email has been rejected" },
            });
          }
      ...
      return response({
        status: 500,
        res: { messsage: "Internal Server Error" },
      });
      ...
      if (PartialOrderItem) {
        ...
        if (AcceptPartialOrderItemUnknownError) {
          return response({
            status: 500,
            res: {
              message: "Internal Server Error At Accepting Partial Order Item",
            },
          });
        }
        ...
        if (InvalidQuantityIdError) {
          return response({
            status: 404,
            res: { message: InvalidQuantityIdError },
          });
        }
        if (Success) {
          const { SendMailSuccess, SendMailUnkownError } = await sendMail({
            message:
              "Thank you for your patience. The available quantity from request quantity will be delievred to you.",
            email: PartialOrderItem.userEmail!,
          });
