...
export async function handleInStock({
  item,
  userId,
  quantityTotal,
}: {
  ...
  const { Quantity, InvalidQuantityIdError, GetQuanitiyUnknownError } =
    await getQuantity(item.quantityId);
  ...
  if (GetQuanitiyUnknownError || !Quantity) {
    return {
      status: 500,
      res: { message: "Unknown Error Occured At getQuantity " },
    };
  }

  const { IsPartialOrderRequest, InvalidQunatityIdError } =
    await checkPartialOrderItem({
      quantity: Quantity,
  ...
  if (IsPartialOrderRequest) {
    const { FreezingProductUnknownError, Freezed } = await handleFreezeQuantity(
    ...
    if (!Freezed || FreezingProductUnknownError) {
      return {
        status: 500,
        res: { message: "Unknown Error Occured At handleFreezeQuantity" },
      };
    }
    const {
    ...
    if (ProductPurchaseRequestError || ProductRequestUnknownError) {
      return {
        status: 500,
        res: {
          message: "Unknown Error Occured While Creating Partial Request Error",
        },
      };
    }
  ...
  const { PurchaseSuccessfull, PurchaseRequestUnknownError } =
    await accpetPurchaseRequest({ item, userId, quantityTotal });
