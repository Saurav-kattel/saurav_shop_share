import { handleInStock } from "@/app/services/api/admin/product/handleInStock";
import { handleOutOfStock } from "@/app/services/api/admin/product/handleOutOfStock";
import { isInStock } from "@/app/services/api/admin/product/isInStock";
...
export async function handlePurchase({
  ...
  try {
    ...
    const { Quantity, InvalidQuantityIdError, GetQuanitiyUnknownError } =
      await getQuantity(filteredData.quantityId);

    if (InvalidQuantityIdError || GetQuanitiyUnknownError || !Quantity) {
      return {
        status: 500,
        res: { message: "invalid quantity id at handle purchase" },
      };
    }
    ...
    const { IsInStock, IsInStockUnknownError } = await isInStock({
      quantityId: filteredData.quantityId,
    });
    if (!IsInStock) {
      ...
      const { status, res } = await handleInStock({
        item: filteredData,
        userId,
