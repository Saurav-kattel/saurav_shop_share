...
const LoginForm = () => {
  ...
  const [response, setResponse] = useState<{
    res: { message?: string | undefined; success?: boolean };
  }>({
    res: {
      message: undefined,
      success: false,
  ...
  return (
    <div className="w-full flex flex-col justify-center items-center m-0 h-[89vh] bg-zinc-200">
      <div className="flex flex-col shadow-xl w-[85vw] py-5  justify-center items-center gap-1">
        ...
        <form
          className=" flex flex-col justify-center items-center gap-1 w-[30vw] outline-none"
          onSubmit={async (e) =>
            await handleLogin({
              e,
              setLoginData,
        ...
        >
          <label className="flex flex-col gap-1">
            <fieldset
              className={`border-[1px] rounded-lg px-2 py-1
                         ${
                           response.res &&
                           (response.res.message?.includes("User") ||
                             response.res.message?.includes("Email"))
          ...
          <label className="flex flex-col gap-1">
            <fieldset
              className={`border-[1px] rounded-md px-2 py-1
                         ${
                           response.res &&
                           (response.res.message?.includes("Password") ||
                             response.res.message?.includes("Wrong"))
                             ? " border-red-800"
                             : "border-zinc-800"
                         }
                        `}
          ...
          {loading ? (
            <span className="w-[14vw] h-[6.5vh] text-center   mt-4">
              Loding...
            </span>
          ) : (
            <Button
              className="bg-slate-900 text-white text-center hover:bg-zinc-200   hover:scale-110 hover:text-slate-900 w-[14vw] mt-4 hover:border-slate-800 border-[1px]"
              variant={"secondary"}
            >
              login{" "}
            </Button>
        ...
        <p className="cursor-pointer">
          <Link href={"/components/user/forget-password"}>forgot password</Link>
        </p>
        <span>Or</span>
        <p>
          Dont have an account?{" "}
          <strong className="cursor-pointer">
            <Link href={"/components/user/register"}>Register</Link>
          </strong>
        </p>
      </div>
    </div>
