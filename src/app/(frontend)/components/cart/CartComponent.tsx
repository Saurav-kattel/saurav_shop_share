...
const CartComponent = () => {
  ...
  return (
    <>
      <section
        className="text-white font-bold hover:border-b-[1px] hover:border-rose-400 hover:text-rose-400"
        onClick={toggleCart}
      >
        CART
      </section>
      ...
      <div
        ref={cartRef}
        className="w-[40vw] h-[100vh] absolute top-0 right-0  transition  translate-x-full flex flex-col  bg-slate-800 overflow-y-scroll shadow-xl shadow-slate-950"
      >
        <button
          onClick={toggleCart}
          className="text-slate-900 w-8 absolute p-4 right-0"
        >
          X
        </button>
        <h3 className="text-rose-500 font-bold  p-4 text-3xl ">
          Shopping Cart
        </h3>
        ...
        <div className="flex  justify-around">
          ...
          <CheckoutButton cartItem={cartItem} toggle={toggleCart} />
        </div>
      </div>
    </>
  );
