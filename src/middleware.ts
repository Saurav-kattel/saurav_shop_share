...
export async function middleware(request: NextRequest) {
    ...
    if (!request.cookies.get("auth") && request.nextUrl.pathname.match("^/admin")) {
        return NextResponse.redirect(new URL('/components/user/login', request.url));
    }
    const admin = await checkAdmin(request.cookies.get('auth')?.value ?? "");

    if ((admin === false || admin === undefined) && request.nextUrl.pathname.match("^/admin")) {
        return NextResponse.redirect(new URL('/unethical/dirty-hacker/bad-human', request.url));
    }
}
